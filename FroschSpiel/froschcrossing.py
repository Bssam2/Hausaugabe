...
class Settings:
    ...
    )  # Pfad zur aktuellen Python-Datei
    IMAGE_PATH = os.path.join(
        FILE_PATH, "images"
    )  # Der Ordner "images" befindet sich im gleichen Verzeichnis wie die Python-Datei
...
class Player:
    ...
    def move(self, keys):
        ...
        if (
            self.is_jumping
            and time.time() - self.last_sprint_time > self.sprint_duration
        ):
            self.is_jumping = False
            self.y = max(
                0, self.y
            )  # Verhindere, dass der Spieler unter den Bildschirm fällt
...
def main():
    ...
    pygame.display.set_caption("Mario croosing game")
    clock = pygame.time.Clock()

    # Hindernisse und Hintergrund laden
    obstacles = [
        MovingObstacle(
            os.path.join(Settings.IMAGE_PATH, "car1.png"), 250, 50, -3, 0, (80, 60)
        ),
        MovingObstacle(
            os.path.join(Settings.IMAGE_PATH, "car1.png"), -350, 50, -3, 0, (80, 60)
        ),
        MovingObstacle(
            os.path.join(Settings.IMAGE_PATH, "car1.png"), 550, 50, -3, 0, (80, 60)
        ),
        MovingObstacle(
            os.path.join(Settings.IMAGE_PATH, "car2.png"), 600, 220, -4, 0, (100, 100)
        ),
        MovingObstacle(
            os.path.join(Settings.IMAGE_PATH, "car3.png"), 200, 250, -4, 0, (80, 60)
        ),
        MovingObstacle(
            os.path.join(Settings.IMAGE_PATH, "car1.png"), 400, 480, -4, 0, (80, 60)
        ),
        MovingObstacle(
            os.path.join(Settings.IMAGE_PATH, "car3.png"), 830, 125, -8, 0, (80, 60)
        ),
        MovingObstacle(
            os.path.join(Settings.IMAGE_PATH, "car3.png"), 200, 550, -4, 0, (80, 60)
        ),
        MovingObstacle(
            os.path.join(Settings.IMAGE_PATH, "car2.png"), 600, 530, -4, 0, (100, 100)
        ),
        MovingObstacle(
            os.path.join(Settings.IMAGE_PATH, "car1.png"), -350, 600, -5, 0, (80, 60)
        ),
        MovingObstacle(
            os.path.join(Settings.IMAGE_PATH, "car1.png"), 300, 600, -5, 0, (80, 60)
        ),
    ]

    background_image = pygame.image.load(
        os.path.join(Settings.IMAGE_PATH, "straße1.png")
    ).convert()
    background_image = pygame.transform.scale(
        background_image, (Settings.WINDOW_WIDTH, Settings.WINDOW_HEIGHT)
    )

    player = Player(os.path.join(Settings.IMAGE_PATH, "spieler1.png"))
